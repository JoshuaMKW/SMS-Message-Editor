...
from PySide6.QtCore import (QByteArray, QDataStream, QIODevice, QMimeData, QRegularExpression, QRect,
                            QModelIndex, QObject, QPersistentModelIndex,
                            QPoint, QRect, QSize, QSortFilterProxyModel, Qt,
                            Signal, Slot)
from PySide6.QtGui import (QAction, QColor, QFont, QImage, QIntValidator, QImage, QCloseEvent, QShowEvent,
                           QMouseEvent, QPainter, QPainterPath, QPaintEvent,
                           QPolygon, QStandardItem, QStandardItemModel,
                           QTextCursor, QTransform)
from PySide6.QtWidgets import (QComboBox, QFileDialog, QFormLayout, QMainWindow,
                               QFrame, QHBoxLayout, QLineEdit, QMenu, QMenuBar,
                               QPlainTextEdit, QSizePolicy,
                               QSplitter, QVBoxLayout, QWidget)

from sms_message_editor import VariadicArgs, VariadicKwargs
from sms_message_editor.gui.settings import SMSMessageEditorSettings
from sms_message_editor.gui.widgets import ABCWidget
from sms_message_editor.gui.widgets.interactivestructs import (
    InteractiveListView)
from sms_message_editor.gui.widgets.listinterface import ListInterfaceWidget
from sms_message_editor.bmg import BMG, RichMessage, SoundID
from sms_message_editor.filesystem import resource_path
from sms_message_editor.iohelper import decode_raw_string
...
class BMGMessagePreviewWidget(QWidget):
    ...
    def render_(self, painter: QPainter):
        ...
        if not self.message or self.message.get_rich_text() == "":
...
class BMGMessageMenuBar(QMenuBar):
    ...
    themeChangeRequested = Signal(bool)
    issueRequested = Signal()
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        themeAction = QAction(otherMenu)
        themeAction.setText("Dark Theme")
        themeAction.setCheckable(True)
        themeAction.toggled.connect(
            self.themeChangeRequested.emit
        )
        self._themeAction = themeAction

        issueAction = QAction(otherMenu)
        issueAction.setText("Report Bug...")
        issueAction.setCheckable(False)
        issueAction.toggled.connect(
            lambda toggled: self.issueRequested.emit()
        )

        otherMenu.addAction(themeAction)
        otherMenu.addSeparator()
        otherMenu.addAction(issueAction)
...
class BMGMessageEditorWindow(QMainWindow):
    class Theme(IntEnum):
        LIGHT = 0
        DARK = 1
    ...
    def __init__(self, *args: VariadicArgs, **kwargs: VariadicKwargs):
        ...
        self.messagePreview = BMGMessagePreviewWidget(parent=self)
    ...
    def populate(self, bmg: BMG):
        self.messageListBox.clear()
        self.messageTextEdit.blockSignals(True)
        self.messageTextEdit.clear()
        self.messageTextEdit.blockSignals(False)

        self.messageMenuBar.set_region_pal(bmg.is_str1_present())
        self.messageMenuBar.set_packet_size(bmg.flagSize)

        for i, message in enumerate(bmg.iter_messages()):
            if message.name == "":
    ...
    @Slot()
    def new_bmg(self):
        self.populate(None, BMG(self.messageMenuBar.is_region_pal(), 12))
        self.messageListWidget.setEnabled(True)
    ...
    def save_bmg(self, path: Optional[Path] = None):
        ...
        bmg = BMG(self.messageMenuBar.is_region_pal(),
                  self.messageMenuBar.get_packet_size())
    ...
    def signal_theme(self, darkTheme: bool):
        if darkTheme:
            self.themeChanged.emit(BMGMessageEditorWindow.Theme.DARK)
        else:
            self.themeChanged.emit(BMGMessageEditorWindow.Theme.LIGHT)

    def showEvent(self, event: QShowEvent) -> None:
        return super().showEvent(event)

    def closeEvent(self, event: QCloseEvent) -> None:
        settings = SMSMessageEditorSettings.get_instance()
        settings.save()
